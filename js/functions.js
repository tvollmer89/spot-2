...
const updateList = (results) => {
  ...
  if(filters.t == "") {
    console.log(`matching: ${new Date(matchingItems[0].pubDate)}`)
...
const renderButton = (obj) => {
  ...
  if(obj.page){link.setAttribute("data-page", obj.page)}
  li.appendChild(link)
  return li
}
 ...
 */
 const addPagerEvents = (links) => {
  links.forEach(link => {
    let p = link.getAttribute("data-page") ? parseInt(link.getAttribute("data-page"), 10) : null
    if(p){
